...
function savePartLead(leadData) {
    return new Promise((resolve, reject) => {
        const { name, email, phone, message, part_type_id, fuel_type, engine_capacity, part_name } = leadData;
        const sql = `INSERT INTO PartLeads (name, email, phone, message, part_type_id, fuel_type, engine_capacity, part_name, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'unattended')`;
        db.run(sql, [name, email, phone, message, part_type_id, fuel_type, engine_capacity, part_name], function(err) {
            if (err) reject(err);
